...
class CosmosConversationClient():
    ...
    async def get_conversations(self, user_id, limit, sort_order = 'DESC', offset = 0):
        ...
        # c.userId = @userId and c.type='conversation'
        # order by c.updatedAt {sort_order}"""

        query = f"""SELECT * FROM c  
                where IS_DEFINED(c.answer) order by c.createdAt desc"""
    ...
    async def create_message(self, uuid, conversation_id, user_id, input_message: dict, answer):
        message = {
            'id': uuid,
            'type': 'message',
            'userId' : user_id,
